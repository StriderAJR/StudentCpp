...
namespace Strings
{
   ...
   // Ф-ция чтения строки с экрана
    char* readString()
    {
        ...
        * под нашу строку.
        */
        char* str = new char[strlen(buf)+1]; // Выделяем ровно столько памяти, сколько "полезных" символов в буфере

       /*
        * Сделать +1 к длине строки требуется, потому что strlen работает вплоть до нулевого байта,
        * а значит это длина строки без самого нулевого байта. Если не прибавить единицу, то после
        * копирования строки может возникнуть heap corruption - повреждение кучи - последний нулевой
        * байт запишется в неразмеченную область памяти.
        */
